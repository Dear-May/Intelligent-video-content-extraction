...
def enhance_video(vid_path, output_path):
    ...
    # 设置预览窗口
    raw_title = 'video'
    enhanced_title = 'enhanced video'
    cv2.startWindowThread()
    cv2.namedWindow(raw_title)
    cv2.namedWindow(enhanced_title)
    # 遍历视频的所有帧
    for i in tqdm(range(total_frame)):  # 使用 tqdm 显示进度条
        ...
        if i % fps == 0:  # 每秒显示一帧
            ...
            cv2.imshow(raw_title, frame)
            cv2.imshow(enhanced_title, tmp_img)
            cv2.waitKey(50)  # 设置显示每帧的时间间隔

    # 关闭预览窗口
    cv2.destroyWindow(raw_title)
    cv2.destroyWindow(enhanced_title)
...
# 使用示例
# watermark_area = (10, 10, 100, 50)  # 水印区域为 (x, y, w, h)
enhanced_vid_path = enhance_video('./video/d5.mp4', 'd5_enhanced.mp4')
txt_path = extract_subtitles_from_video(enhanced_vid_path)
print(f"增强视频完成，文本文件生成完成：{txt_path}")
