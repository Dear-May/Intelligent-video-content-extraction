...
def extract_subtitle_area(frames, height, h_ratio_start=0.86, h_ratio_end=0.94):
    ...
    create_and_clear_folder(process_dir)
    ...
    for idx, frame in enumerate(frames):
        ...
        cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_gray.jpg"), gray)

        # 双边滤波降噪
        filtered = cv2.bilateralFilter(gray, 9, 75, 75)
        cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_filtered.jpg"), filtered)

        # Canny边缘检测
        edges = cv2.Canny(filtered, 100, 200)
        cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_edges.jpg"), edges)

        # 形态学操作：膨胀文字，以便更好地连接文字的轮廓
        kernel = np.ones((3, 3), np.uint8)
        dilated = cv2.dilate(edges, kernel, iterations=1)
        cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_dilated.jpg"), dilated)
        ...
        if contours:
            ...
            cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_mask.jpg"), mask)
            # 应用掩模获取最终的文字图像
            final_text_image = cv2.bitwise_and(filtered, filtered, mask=mask)
            cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_final_text_image.jpg"), final_text_image)
...
def process_image(image):
    ...
    cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_binary.jpg"), binary)
    ...
    cv2.imwrite(os.path.join(process_dir, f"frame_{idx}_morph.jpg"), morph)
...
def crop_to_text_area(image):
    ...
    # 应用二值化处理，识别白色区域
    _, binary = cv2.threshold(image, 200, 255, cv2.THRESH_BINARY)
    cv2.imwrite(os.path.join(base_dir, "process", f"frame_{idx}_binary_crop.jpg"), binary)

    # 使用形态学操作去噪点
    kernel = np.ones((5, 5), np.uint8)
    morph = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)
    cv2.imwrite(os.path.join(base_dir, "process", f"frame_{idx}_morph_crop.jpg"), morph)
    ...
    cv2.imwrite(os.path.join(base_dir, "process", f"frame_{idx}_cropped.jpg"), cropped_image)
...
def remove_text_with_inpainting(frame, mask):
    ...
    cv2.imwrite(os.path.join(base_dir, "process", f"frame_{idx}_inpainted.jpg"), inpainted_frame)
...
def process_frame(frame, template, top_left, bottom_right):
    ...
    cv2.imwrite(os.path.join(base_dir, "process", f"frame_{idx}_clear_mask.jpg"), mask)
